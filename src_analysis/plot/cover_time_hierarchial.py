...
def load_tree_summary(path: str) -> pd.DataFrame:
    """
    从 cover_time_results_tree_summary.csv 读取并清洗数据，返回 tidy DataFrame：
    列包括：
      - method_raw: 原始方法名
      - method:     清洗后的方法名（更好看）
      - seconds
    ...
    # 从第一列列名解析第一个图的 n, m，比如 "tree_(n=31,m=30)"
    first_col_name = raw_df.columns[0]
    m0 = re.match(r"tree_\(n=(\d+),m=(\d+)\)", first_col_name)
    if m0:
    ...
    for _, row in raw_df.iterrows():
        ...
        # 检测块头，例如 'tree_(n=63,m=62)'
        m = re.match(r"tree_\(n=(\d+),m=(\d+)\)", first)
        if m:
            current_n = int(m.group(1))
            current_m = int(m.group(2))
            # 下一行是 graph,tree_xxx,tree_xxx,edge_cover_time 的 header，跳过
...
def plot_vertex_cover_vs_n(df: pd.DataFrame):
    ...
    plt.title("Vertex cover time vs n on tree graphs")
    plt.legend()
    plt.tight_layout()
    plt.savefig("tree_vertex_cover_vs_n.pdf")


import matplotlib.pyplot as plt
import numpy as np

def plot_edge_cover_vs_n(df, fname: str = "tree_edge_cover_vs_n.pdf"):
...
def plot_runtime_vs_n(df: pd.DataFrame, logy: bool = True):
    ...
    if logy:
        plt.yscale("log")
        plt.title("Runtime vs n on tree graphs (log scale)")
    else:
        plt.title("Runtime vs n on tree graphs")
    plt.legend()
    plt.tight_layout()
