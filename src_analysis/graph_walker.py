...
def as_text(walks, restarts, G, include_neighbors=True, verbose=True):
    ...
    if include_neighbors:
        ...
        indptr = A.indptr.astype(np.uint32)
        indices = A.indices.astype(np.uint32)
        named_walks, walks, restarts, neighbors = _anonymize_neighbors(walks, restarts, indptr, indices)
        walks_text = _as_text_neighbors(named_walks, restarts, neighbors)
    else:
...
def random_walks_precomputed_probs(
    indptr, indices, data,
    n_walks=10,
    walk_len=10,
    p=1, q=1, alpha=0, k=None,
...
def as_text_peptides(
    ...
    if include_neighbors:
        ...
        indices = A.indices.astype(np.uint32)
        named_walks, walks, restarts, neighbors = _anonymize_neighbors(walks, restarts, indptr, indices)
        walks_text = _as_text_neighbors_peptides(named_walks, restarts, neighbors, walks, node_attr_str, indptr_edge_attr, indices_edge_attr, edge_attr_str)
    else:
        named_walks = _anonymize(walks)
...
def as_text_arxiv(
    ...
    if include_neighbors:
        named_walks, walks, restarts, neighbors = _anonymize_neighbors(walks, restarts, indptr_undirected, indices_undirected)
        backwards = _parse_directions_neighbors(walks, restarts, neighbors, indptr_directed, indices_directed)
        walks_text = _as_text_neighbors_arxiv(named_walks, backwards, restarts, neighbors, walks, title, abstract, input_title, input_abstract, input_label)
    else:
        named_walks = _anonymize(walks)
...
def as_text_amazon(
    ...
    if include_neighbors:
        named_walks, walks, restarts, neighbors = _anonymize_neighbors(walks, restarts, indptr, indices)
        walks_text = _as_text_neighbors_amazon(named_walks, restarts, neighbors, walks, product, input_product, input_label)
    else:
        named_walks = _anonymize(walks)
